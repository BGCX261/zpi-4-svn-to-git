...
import czytnik.DodajacyOdciskiModule;
...
public class Applet extends JApplet {
  ...
  private JButton btn = new JButton("Dodaj odcisk palca dla uzytkownika");
  ...
  public void init() {
    ...
    if (uzytkownikId == null)
      uid = 7;// domyślna wartość
    else
      uid = Integer.parseInt(uzytkownikId);
    liczbaPrzeciagniec = 3;
    final Injector inject = Guice.createInjector(new AuthorizationModule(),
        new DodajacyOdciskiModule(liczbaPrzeciagniec), new AlgorytmModule(),
        new SocketModule(), new ServicesModule(), new BazaDanychModule());
    db = inject.getInstance(DatabaseFacade.class);
    obserwator = inject.getInstance(ObserwatorCzytnika.class);
  ...
  private final class FingerprintAddingWorker extends
    ...
    protected Boolean doInBackground() throws Exception {
      ...
      return db.dodajUzytkownikowiMetodeAutoryzacji(uid, hash);
    }
